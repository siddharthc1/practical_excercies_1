...
hive -e "use practical_excercise_1;"
...
hive -e "create table user_total(time_ran int, total_users int , users_added int);"

hive -e "INSERT into user_total select 1528136917, tab.co , tab1.dif from (select count(1) as co from activitylog where timestamp between 0 and 1528136917) as tab,
(Select d1 - d2  as dif from
(select count(1) as d1 from activitylog where timestamp between 0 and 1528136917) as a,
(select c.total_users as d2 from (select * from user_total order by time_ran desc limit 1) as c) as b) as tab1;"
